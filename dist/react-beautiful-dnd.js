(function (global, factory) {
	...
	*/
	...
	var getDroppableOver = (function (_ref) {
	  ...
	  var maybe = toDroppableList(droppables).filter(function (droppable) {
	    ...
	    return a.subject.active.width * a.subject.active.height - b.subject.active.width * b.subject.active.height;
	  });
	...
	var warning$1 = function warning(message) {
	  var _console;
	...
	*/
	...
	var DroppableDimensionPublisher = function (_React$Component) {
	  ...
	  _proto.componentWillUnmount = function componentWillUnmount() {
	    if (this.dragging) {
	      warning$1('unmounting droppable while a drag is occurring');
	      this.dragStopped();
	    }
	...
	var tryRestoreFocus = function tryRestoreFocus(id, draggableRef) {
	  ...
	  if (!dragHandleRef) {
	    warning$1('Could not find drag handle in the DOM to focus on it');
	    return;
	  }
	  ...
	  var windowBindings = [{
	    ...
	    fn: function fn(event) {
	      if (event.webkitForce == null || MouseEvent.WEBKIT_FORCE_AT_FORCE_MOUSE_DOWN == null) {
	        warning$1('handling a mouse force changed event when it is not supported');
	        return;
	      }
	  ...
	  _proto.componentDidUpdate = function componentDidUpdate(prevProps) {
	    ...
	    if (isBeingDisabled) {
	      this.sensors.forEach(function (sensor) {
	        ...
	        if (wasDragging) {
	          warning$1('You have disabled dragging on a Draggable while it was dragging. The drag has been cancelled');
